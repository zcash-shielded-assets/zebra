...
use crate::methods::{
    get_block_template_rpcs::get_block_template::calculate_transaction_roots,
};
...
pub fn create_genesis_block(
...
) -> Result<CreateGenesisResponse> {
    // --- Parse network magic ---
    let magic_bytes = hex::decode(network_magic_hex).map_err(|e| {
        jsonrpc_core::Error::invalid_params(format!(
            "invalid network_magic hex: {e}"
        ))
    })?;
    if magic_bytes.len() != 4 {
        return Err(jsonrpc_core::Error::invalid_params(
            "network_magic must be exactly 4 bytes (8 hex chars)",
        ));
    }
    ...
    let coinbase_input = transparent::Input::new_coinbase(
        block::Height(0),
        Some(GENESIS_COINBASE_DATA.to_vec()),
    ...
    let coinbase_output = transparent::Output::new_coinbase(
    ...
    if !disable_pow {
        ...
        let solve_result = Solution::solve_genesis(header, || Ok(()))
            .map_err(|e| jsonrpc_core::Error {
                ...
                message: format!("equihash solver error: {e}"),
                data: None,
            })?;

        let solved_headers = solve_result.into_vec();
...
mod tests {
    ...
    fn create_zsa_testnet_1_genesis_mined() {
        ...
        println!("Network Upgrade: NU6 @ height 1");
        println!();
